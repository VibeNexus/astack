...
> **文档状态：** 开发中（基于 Spec v1.1；PR1 进行中）
>
> 创建于 2026-04-22，分支 main
>
> 前置迭代：v0.5（Subscription Bootstrap）、v0.4（SystemSkill 领域分离）
>
> **评审归档：** [Iteration0.7_LocalSkills_REVIEW.md](./Iteration0.7_LocalSkills_REVIEW.md)
>
...
| PR2 | `LocalSkillService` + `LocalSkillRepository` + 单测 | 待开始 | |
| PR3 | HTTP 端点 + Zod schemas + SSE 事件 | 待开始 | |
| PR4 | `scanRaw` 过滤 + `autoAdoptFromUnmatched` 集成 | 待开始 | |
| PR5 | 前端（api + Panel + Drawer + tab + UnmatchedEmptyState 重构）| 待开始 | |
| PR6 | E2E + 文档回写 | 待开始 | |
