...
- `Linked tools` 没有任何说明，三个固定按钮（cursor/codebuddy/windsurf）+ 灰色状态点，用户不知道"我点了什么"。
...
1. `packages/web/src/pages/ProjectDetailPage.tsx` 重写为 Tabs 布局（Subscriptions / Linked Tools / Sync History / Settings）
2. 新 primitive：`Tabs`（键盘可达 + searchParams 驱动）、`Drawer`（右侧滑出 + focus trap + Esc 关闭）
3. 新业务组件：`BrowseSkillsDrawer`、`SyncResultCard`、`SubscriptionRow`、`ToolLinkCard`、`ProjectHeader` + `SummaryBar`
...
9. `ToolLink` schema 扩展 `target_path: string | null` + `broken_reason: "target_missing" | "not_a_symlink" | "permission_denied" | null`；`SymlinkService` 读 `readlink` 填充
...
export interface ToolLink {
  ...
  status: ToolLinkStatus;      // "active" | "broken" | "removed"
  broken_reason: ToolLinkBrokenReason | null;  // [NEW]
...
export type ToolLinkBrokenReason = (typeof ToolLinkBrokenReason)[keyof typeof ToolLinkBrokenReason];
```
...
function useProjectActions(projectId: number, reload: () => Promise<void>) {
  ...
  return {
    ...
    addLink: (tool: string) =>
      runAction(() => api.createToolLink(projectId, { tool_name: tool }),
        { okMsg: `Linked ${tool}`, errMsg: "Link failed" }),
    removeLink: (tool: string) =>
      runAction(() => api.deleteToolLink(projectId, tool),
...
| PR | Lane | 状态 |
|---|---|---|
| PR0 — Playwright 脚手架 | C | ⏳ pending |
| PR1 — `ui.tsx` → `ui/` 目录 + `useProjectActions` hook（纯重构）| B | ⏳ pending |
| PR2 — `GET /api/projects/:id/sync-logs` + Repository 方法 | A | ⏳ pending |
| PR3 — `ToolLink.target_path` + `broken_reason` + SymlinkService 扩展 | A | ⏳ pending |
| PR4 — batch subscribe 改 partial-success + CLI 同步 | A | ⏳ pending |
| PR5 — `Tabs` + `Drawer` primitive | B | ⏳ pending |
| PR6 — `ProjectDetailPage` 重写（Tabs + SummaryBar + Subscriptions tab） | C | ⏳ pending |
| PR7 — `BrowseSkillsDrawer` + `SyncResultCard` | C | ⏳ pending |
| PR8 — Linked Tools tab + Sync History tab + Settings tab | C | ⏳ pending |
| PR9 — Mobile 响应式 + CommandPalette 扩展 + a11y 专项测试 | C | ⏳ pending |
...
### PR3 — `ToolLink.target_path` + `broken_reason` + SymlinkService 扩展

**范围：** `packages/shared`、`packages/server`、`packages/web`（只改类型，不触前端 UI）。

**改动：**

- `packages/shared/src/domain.ts`：`ToolLink` 接口加 `target_path`、`broken_reason`；新增 `ToolLinkBrokenReason` 枚举
- `packages/shared/src/schemas/links.ts`：`ToolLinkSchema` 同步扩展
...
**测试要点：**
- `useSearchParams` 驱动：`?tab=tools` → Linked Tools 面板；无 → 默认 subscriptions
...
### PR8 — Linked Tools tab + Sync History tab + Settings tab

**范围：** `packages/web/src/components/project/`、`pages/ProjectDetailPage.tsx`。

**改动：**

- `components/project/ToolLinkCard.tsx`：一条 tool link，显示 status dot + tool_name + `→ target_path` + `created X ago` + broken_reason（如有）+ [Unlink] / [Re-link]
- `components/project/LinkedToolsPanel.tsx`：所有 tool links 的 ToolLinkCard 列表 + 底部 `[+ Link a tool ▾]` dropdown（cursor / codebuddy / windsurf / Custom path…）
...
**Custom path dropdown item：** 点击后再开一个小 dialog，用 `PathAutocomplete`（已有）选目标目录；调 `api.createToolLink(projectId, { tool_name: "custom", dir_name: "<basename>" })`——**注意：当前 `CreateToolLinkRequest` 只接 `tool_name`**。可能需要后端支持 `dir_name` 覆盖，或者把 Custom 放 out-of-scope 延后。
...
- [ ] Linked Tools tab 显示真实 symlink target path；broken 状态能说明原因
...
| R4 | `ToolLink.target_path` 每次 readlink 可能有 I/O 开销 | 目前每个项目 ≤ 3 个 link，I/O 可忽略；如未来 link 数量爆炸再加 debounced cache |
