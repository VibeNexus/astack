...
- `packages/server/src/db/schema.ts`：
  - `SCHEMA_VERSION = 2`
  - 保留 `SCHEMA_DDL` 改名 `SCHEMA_DDL_V1`
  - 新增 `MIGRATION_V2` 字符串（仅包含 ALTER 和重建表逻辑）
- `packages/server/src/db/connection.ts: migrate(db)`：
  - 按版本分派：
    ```ts
    export function migrate(db: Db): void {
      ...
      if (getSchemaVersion(db) < 2) {
        migrateV1ToV2(db, dbPath);  // 含备份 + pragma + 事务 + foreign_key_check
        setVersion(db, 2);
      }
    }
    ```
  - `migrateV1ToV2` 关键流程（见下）

- **关键：migrateV1ToV2 的执行顺序**（不能乱，SQLite 特定约束）：

  ```ts
  function migrateV1ToV2(db: Db, dbPath: string): void {
    // 1. 文件级备份（:memory: 跳过）
    if (dbPath !== ':memory:') {
      fs.copyFileSync(dbPath, `${dbPath}.bak.v1`);
    }
    ...
    try {
      ...
      db.exec(`
        ...
        INSERT INTO skills_new (id, repo_id, type, name, path, description, version, updated_at)
          SELECT id, repo_id, type, name, path, description, version, updated_at FROM skills;
        DROP TABLE skills;
        ALTER TABLE skills_new RENAME TO skills;
        CREATE INDEX idx_skills_repo ON skills(repo_id);
      `);

      // 3c. skill_repos 新字段（不加 CHECK，zod 保证）
      db.exec(`
        ALTER TABLE skill_repos ADD COLUMN status TEXT NOT NULL DEFAULT 'ready';
        ALTER TABLE skill_repos ADD COLUMN scan_config TEXT;  -- JSON, NULL = DEFAULT_STANDARD
      `);

      // 3d. 新表 seed_decisions（OV-7 修正，不塞 meta）
      db.exec(`
      ...
      db.exec('ROLLBACK');
      // 恢复备份
      if (dbPath !== ':memory:' && fs.existsSync(`${dbPath}.bak.v1`)) {
        // 需要先关 db 才能覆盖文件，migration 调用者要处理这个
      }
      throw err;
    }
    ...
    if (violations.length > 0) {
      throw new Error(`FK violations after v2 migration: ${JSON.stringify(violations)}`);
    }
  }
  ```

  **⚠️ 字段名修正**：outside voice 评审中我用的是 `layout_config`，采纳 OV-T2 后改为 `scan_config`（与 scanner 的 `ScanConfig` 类型对齐，不再用 layout 这个词）。

- `packages/server/src/db/repos.ts` / `packages/shared/src/domain.ts` / schemas：扩展 `SkillRepo` 类型含 `status: 'ready'|'seeding'|'failed'` 和 `scan_config: ScanConfig | null`
- `packages/server/src/db/skills.ts` 扩展 upsert 参数含 `description: string | null`，scanAndUpsert 传入 frontmatter 读出的 description

**测试要点（强制，T-1 定）**：

新建 `test/db-migration.test.ts`：

- **v1 fixture 完整性**：手写 v1 SCHEMA_DDL（复制自 schema.ts 历史版本），填充 `skill_repos`（2 行）+ `skills`（5 行）+ `sync_logs`（3 行 FK 到 skill_id）+ `subscriptions`（2 行 FK 到 skill_id + project_id）
- 跑 `migrate` 到 v2
- 断言：
  - `getSchemaVersion(db) === 2`
  - `SELECT COUNT(*) FROM skills === 5`（不丢）
  - `SELECT COUNT(*) FROM sync_logs === 3`（FK 指向重建后的 skills 没有级联删）
  - `SELECT COUNT(*) FROM subscriptions === 2`
  - 所有 skills.description IS NULL（v1 没这个字段，迁移后为 null 而非丢数据）
  - `PRAGMA foreign_key_check` 返回 0 行
  - 重建后的 skills 表新 type 可写 'agent'、写 'wrong' 被拒
- 幂等性：跑 migrate 两次不报错
- 备份文件检查：非 `:memory:` 场景下 `.bak.v1` 文件存在
- 新 `seed_decisions` 表 CREATE + INSERT 工作

**验收**：
- 所有现有 db 测试通过
- 新 migration 测试全绿（尤其 FK 不丢数据）
- `pnpm test:coverage` 门槛保持（lines 90% / branches 85%）
